    ...
    private void BindGrid(string strQueryStatus, string contractorid)
    {
        try
        {
            ...
            foreach (DataRow rows in contractorcloseoutDt.Rows)
            {
                ...
                RowsDt += "<a href=\"Edit.aspx?ID=" + rows[0] + "\" class='on-default edit-row'><i class='fa fa-pencil'></i></a>";
                RowsDt += "<a href=\"Delete.aspx?ID=" + rows[0] + "\" class='on-default remove-row'><i class='fa fa-trash-o'></i></a>";
                RowsDt += "</td>";
                RowsDt += "<td style='display:none;'>" + rows["contractorclose_id"] + "</td>";
                RowsDt += "<td style='display:none;'>" + rows["contractor_id"] + "</td>";
